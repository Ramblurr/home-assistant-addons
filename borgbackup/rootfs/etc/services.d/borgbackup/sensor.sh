...
declare BORG_STATUS=(IDLE RUNNING SUCCEEDED FAILED)
declare SENSOR_NAME="sensor.borgbackup"
declare SENSOR_URL="/core/api/states/${SENSOR_NAME}"
declare STORAGE_FILE="/backup/.borgbackup.sensor"
...
function update_sensor {
    ...
    if bashio::var.has_value "${all}"; then
        ...
        if response=$(eval "${SMB} -c 'cd \"${TARGET_DIR}\"; ls'"); then
            BACKUPS_REMOTE=$(echo "$response" | grep -E '\<([0-9a-f]{8}|borgbackup_.*)\.tar\>' | wc -l)
        fi

        if [ "$CURRENT_STATUS" = "${BORG_STATUS[2]}" ]; then
            TOTAL_SUCCESS=$((TOTAL_SUCCESS + 1))
            LAST_BACKUP=$(date +'%Y-%m-%d %H:%M')
        elif [ "$CURRENT_STATUS" = "${BORG_STATUS[3]}" ]; then
    ...
    if ! response=$(ha_post_sensor "$data"); then
...
# ------------------------------------------------------------------------------
function ha_post_sensor {
    local data=${1}
    local status
    local response
    echo "sensor.sh SUPERVISOR TOKEN: ${SUPERVISOR_TOKEN}"
    curl -v -X GET \
    ...
    bashio::log.debug "API Endpoint: ${__BASHIO_SUPERVISOR_API}${SENSOR_URL}"
